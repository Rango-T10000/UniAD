...
model = dict(
    ...
    freeze_pts_bbox_head=True, #把整个Bevformer给frozen
    freeze_uniad=True,         #把整个uniad给frozen
    score_thresh=0.4,
...
data = dict(
    samples_per_gpu=1,
    workers_per_gpu=1,   #原本是8，表示每个 GPU 将分配 8 个数据加载进程来并行加载数据
    train=dict(
        type=dataset_type,
...
# work_dir = '/home2/wzc/UniAD/projects/work_dirs/stage3_IMU'